...
$DelegateName = "liskpro.com"
$LSKunit = 100000000
$exportcsvpath = "c:/temp/distribution.csv"
$exportxmlpath = "c:/temp/distribution.xml"

#get delegate publickey
$DelegatePubKey = (Invoke-WebRequest ($APIendpoint + "delegates/get?username=" + $DelegateName) | ConvertFrom-Json).delegate.publickey

#get delegate voters list with at least $MinLskBalance balance (see script var)
$voters = (Invoke-WebRequest ($APIendpoint + "delegates/voters?publicKey=" + $DelegatePubKey) | ConvertFrom-Json).accounts |  where-object {[int64]$_.balance -ge $($MinLskBalance*100000000)}
